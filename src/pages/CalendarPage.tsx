...
from "@/components/dashboard/ActivityToastListener";

import {
  ActivityFilterBar
}

from "@/components/dashboard/ActivityFilterBar";
...
from "@/components/activities/NewActivityButton";

import {
  ActivityDialog
}

from "@/components/dashboard/ActivityDialog";

import {
  ActivityList
}

from "@/components/activities/ActivityLists";
...
from "@/components/dashboard/AISuggestions";
...
const CalendarPage: React.FC=()=> {
  ...
  =useAuth();
  const [activeTab,
  setActiveTab]=useState("timewisely");
  ...
  =useToast();
  const [oauthComplete,
  setOAuthComplete]=useState(false);
  const [isGoogleConnected,
  setIsGoogleConnected]=useState(localStorage.getItem("googleCalendarConnected")==="true"
  );
  ...
  useEffect(()=> {
      ...
      if ((token || oauthError || code) && !oauthComplete && !alreadyProcessed) {
        ...
        if (token) {
          ...
          if (getCurrentUser) {
            getCurrentUser() .then(()=> {
                console.log("User data refreshed after Google Calendar connection",
                );
              }
          ...
          success("Google authentication successful, setting up calendar access...",
          );
  ...
  useEffect(()=> {
      ...
      const params=new URLSearchParams(location.search);
      const isOAuthCallback=params.has("token") || params.has("code") || params.has("error");
  ...
  const [view,
  setView]=useState<View>("week");
  const [date,
  setDate]=useState(new Date());
  const [search,
  setSearch]=useState("");
  const [filterColor,
  setFilterColor]=useState<string | null>(null);
  const [filterAllDay,
  setFilterAllDay]=useState<null | boolean>(null);

  // Helper: convert {startTime, endTime} to {start, end}
  const slotToLegacy=useCallback((slot: {
  ...
  const mergedActivities=useMemo(()=> {
      ...
      const taskItems: Activity[]=tasks.map((t)=> ( {
            ...
            color: "#7D2A2A", // Default color for tasks
            allDay: true,
  ...
  return (<div className="flex flex-col h-screen"> <ActivityToastListener error= {
      activityError
    }
    ...
    > <div className="flex items-center justify-between p-2"> <div className="flex items-center"> <TabsList className="mr-4"> <TabsTrigger value="timewisely">TimeWisely Calendar</TabsTrigger> <TabsTrigger value="activities">Activities</TabsTrigger> {
      isGoogleConnected && (<TabsTrigger value="google">Google Calendar</TabsTrigger>)
    }

    </TabsList> {
      activeTab==="timewisely"&& (<ActivityFilterBar search= {
          search
        }
      ...
      (activeTab==="timewisely"|| activeTab==="activities") && (<NewActivityButton onOpenModal= {
          ()=> {
            handleSelectSlot( {
                startTime: new Date(),
    ...
    </div> <div className="flex-1 overflow-hidden"> <TabsContent value="timewisely"className="h-full overflow-auto"> <Card className="h-full"> <CardContent className="h-full overflow-auto"> <ScheduleCalendar className="modern-calendar"
    ...
    /> <AISuggestions /> </CardContent> </Card> </TabsContent> <TabsContent value="activities"className="h-full overflow-auto"> <Card className="h-full"> <CardContent className="p-6"> <div className="mb-6"> <h1 className="text-3xl font-bold">Activities</h1> <p className="text-muted-foreground text-base"> Manage your activities and track your progress. </p> </div> <ActivityList activities= {
      activities
    }
    ...
    /> </CardContent> </Card> </TabsContent> <TabsContent value="google"className="h-full overflow-auto p-6"> <GoogleCalendar /> </TabsContent> </div> </Tabs> <ActivityDialog isOpen= {
      isActivityModalOpen
    }
