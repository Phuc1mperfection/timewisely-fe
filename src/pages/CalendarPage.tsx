...
const CalendarPage: React.FC = () => {
    ...
    if ((token || oauthError || code) && !oauthComplete && !alreadyProcessed) {
      ...
      if (token) {
        ...
        setTimeout(
          () => {
            if (getCurrentUser) {
              getCurrentUser()
                .then(() => {
                  console.log(
                    "User data refreshed after Google authentication",
                  );
                })
                .catch((error: unknown) => {
                  console.error("Failed to refresh user data:", error);
          ...
          2000,
        ); // Give backend a moment to process the code
      } else if (oauthError) {
  ...
  const { tasks, toggleComplete: toggleTaskComplete, deleteTask } = useTasks();
